...
function partitionTwo<T>(list, left, right, f: (arg1: T, arg2: T) => boolean): number
{
    ...
    while (left_1 < right_1) {
        ...
        while (!f(list[left_1], pivot) && left_1 < right) {left_1 +=1;}
        /*right check point keep moving to head until it finds a number less than pivot */
        while (f(list[right_1], pivot) && right_1 > left) {right_1 -=1;}
        ...
        if (left_1 < right_1) {swap(list, left_1, right_1);}
    }
...
function quicksortTwo<T>(list, left, right, comparision: (arg1: T, arg2: T) => boolean)
{
    if (left < right) {
...
function quicksortThree<T>(list, left, right, comparision: (arg1: T, arg2: T) => boolean, partition: (list, left, right, f: (arg1: T, arg2: T) => boolean) => number)
{
    if (left < right) {
...
function comparisionIntAsc<T>(v1: T, v2: T): boolean {
    if (v1 >= v2)
        return true;
    else
        return false;
...
function comparisionIntDesc<T>(v1: T, v2: T): boolean {
    if (v1 < v2)
        return true;
    else
        return false;
...
function comparisionStrLen(v1: string, v2: string): boolean {
